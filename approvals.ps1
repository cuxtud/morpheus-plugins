...
<#
    ...
    .EXAMPLE
        ...
        2. Command Example:
            pwsh -file 'path/file.ps1' -morphURL '<%=morpheus.applianceUrl%>' -serviceBearer "<%=cypher.read('secret/Bearer')%>" -checkTime '<%=customOptions.checkTime%>'

    .ExitCodes
        0 - Completed without Issue
        1 - No Approvals Pending
        2 - No NEW Approvals Pending
#>
...
Param (
    ...
    [Parameter(Mandatory = $true)]$checkTime # Note: Set this to the same (minute) time interval of your Morpheus Job
)
...
### Script ###
$Approvals = (Invoke-WebRequest -Method Get -Uri ($MorphURL + 'api/approvals/') -Headers $Header).content | ConvertFrom-Json | select -ExpandProperty approvals
    ...
    foreach ($Approval in $Approvals) {
        if ($Approval.status -like "*requested*") {
            ...
            if ($approvalDate.AddMinutes($checkTime) -ge $Date) {            
                ...
                <p style="margin-left:2.5em">Cores: $($Instance.maxCores * $Instance.maxCpu),</p> `
                <p style="margin-left:2.5em">Memory: $($Instance.maxMemory / 1024 / 1024 / 1024) GB,</p> `
